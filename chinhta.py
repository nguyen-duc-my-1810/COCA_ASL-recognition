...
model = load_model("D:\\Project AI\\left_hand_model.h5")
...
file_path = "D:\\Project AI\\Sign-Language-master (1)\\Sign-Language-master\\Index_to_letters.json"
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        if predicted_character == stable_character:
            ...
            if current_stable_frames == STABLE_FRAMES_THRESHOLD:
                ...
                if predicted_character == "space":
                    if previous_character == "space":
                        ...
                        text_before_space = " ".join(words_before_space)  # Kết hợp các từ thành chuỗi
            ...
            (text_width, _), _ = cv2.getTextSize(accumulated_word, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)

            current_time = time.time()
            if current_time - last_time > 0.5:  # Thay đổi mỗi 0.5 giây
                show_cursor = not show_cursor
                last_time = current_time

            if show_cursor:
                cursor_position = (10 + text_width, 25)
                cv2.line(stable_window, cursor_position, (cursor_position[0], cursor_position[1] + 30), (0, 0, 0), 1)
